    ...
    public class QcSplitStudyCmdService : QcStudyCmdService<DataCorrection.V1.SplitStudyParameter>
    {
        ...
        public override async Task<bool> Execute()
        {
            try
            {
                ...
                // 確認是否有Mapped過，如果有，則不允許拆解
                await ValidIsMapped(Data.StudyInstanceUID);
                ...
                foreach (DicomSeriesUniqueIdentifiersTable _seTable in TobeDcmStudyUidTable.DetailElements)
                {
                    ...
                    if (_seTable.ReferencedSeriesInstanceUID.Value == "" ||
                    ...
                    foreach (DicomImageUniqueIdentifiersTable imageTable in _seTable.DetailElements)
                    {
                        ...
                        //更換UID資料
                        if (ReplaceDicomUniqueIdentityUID(dcmFile.Dataset, _seTable.ReferencedStudyInstanceUID.Value,
                        ...
                        //建立拆解回復的影像表格
                        DicomImageUniqueIdentifiersTable
                            splitToImageUidTable = CreateSplitRestoredImageTable(_imgTable);
        ...
        /// <returns></returns>
        protected DicomStudyUniqueIdentifiersTable CreateSplitRestoredStudyTable(
            DicomSeriesUniqueIdentifiersTable originalSeriesTable)
        {
        ...
        /// <param name="originalSeriesTable"></param>
        /// <returns></returns>
        protected DicomSeriesUniqueIdentifiersTable CreateSplitRestoredSeriesTable(
            DicomSeriesUniqueIdentifiersTable originalSeriesTable)
        {
        ...
        /// <param name="originalSeriesTable"></param>
        /// <returns></returns>
        protected DicomImageUniqueIdentifiersTable CreateSplitRestoredImageTable(
            DicomImageUniqueIdentifiersTable originalImageTable)
        {
        ...
        /// <returns></returns>
        protected bool ReplaceDicomUniqueIdentityUID(DicomDataset dataset, string studyUID, string seriesUID,
            string sopUID)
        {
            try
            {
                //原生的PatientID由ReferencedPatientAliasSequence取得
                string patientId = GetReferencedUidFromDataset(dataset, DicomTag.ReferencedPatientAliasSequence,
                    DicomTag.PatientID);
                ...
                string value = dicomOperator.GetDicomValueToStringWithGroupAndElem(dataset,
                ...
                foreach (var fieldToDcmTagMap in MergeSplitMappingTagTable)
                {
                    ...
                    dicomOperator.WriteDicomValueInDataset(dataset, new DicomTag(t_group, t_element), updateValue,
                        isUtf8);
                }
            }
            catch (Exception ex)
        ...
        protected async Task UpdateStudyMaintainStatusToDatabase()
        {
            ...
            DcmStudyQCStatusTable.SetInstanceUIDAndMaintainType(Data.StudyInstanceUID,
        ...
        protected async Task<bool> ValidIsMapped(string studyUid)
        {
            try
            {
                List<PairDatas> pkeys = new()
                ...
                var table = await DbQryService.BuildTable("DicomStudy", pkeys, nkeys, "")
                    .GetDataAsync();

                if (!table.DBDatasets.Any()) throw new Exception($"Failed to query study table on {studyUid}");

                // Check if the study from which data is being merged has been mapped
                var isMapped = TableElementHelper.FindNormalKeyField(table, "Mapped").Value.Trim();

                // If the study has been mapped, throw an exception
                if (!string.IsNullOrEmpty(isMapped) && int.Parse(isMapped) == 1)
